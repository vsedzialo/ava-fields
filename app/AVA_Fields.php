...
class AVA_Fields {
    ...
    private function __construct() {
        ...
        // Ajax hooks
        add_action( 'wp_ajax_avaf-save', [ '\AVAFields\Core\Options', 'save' ] );
        add_action( 'wp_ajax_nopriv_avaf-save', [ '\AVAFields\Core\Options', 'save' ] );
    }
    ...
    public function make( $type, $params ) {
        ...
        if ( !empty($params['access']) ) {
            $ac = new \AVAFields\Core\Access( $params['access'] );
        }
        */
        ...
        switch($type) {
            ...
            case 'settings-page':
            case 'post-type-page':
                $container_class = '\AVAFields\Containers\Page';
                break;
            case 'user-meta':
                $container_class = '\AVAFields\Containers\User_Meta';
                break;
            case 'options-general':
                $container_class = '\AVAFields\Containers\Options';
                break;
            default:
    ...
    public static function field( $type, $params=[] ) {
        ...
        if ( ! class_exists( $class_name ) ) {
            $file = AVA_Fields()->dir('fields') .  $type . '/' . $type . '.php';
            if ( file_exists( $file ) ) {
                require_once( $file );
            }
        }
