<?php
namespace AVAFields\Core\Abstracts;

use AVAFields\Core\Options;
...
abstract class Field {
    ...
    public function load_styles() {
        if (file_exists($this->field_dir . 'assets/styles.css')) {
            AVA_Fields()->add_style( 'avaf-field-'.$this->type, $this->field_dir . 'assets/styles.css' );
        }
    }
    ...
    public function get_value( $key, $key2 = null ) {
        ...
        if ($this->container_id) {
            $fields_options = AVA_Fields()->container( $this->container_id )->Options->fields_options;
    ...
    */
    ...
    public function render() {
        ...
        if ( AVA_Fields()->section( $this->container_id, $this->section_id ) instanceof \AVAFields\Sections\Page ) {
            $html = $this->renderPageField();
        } else if ( AVA_Fields()->section( $this->container_id, $this->section_id ) instanceof \AVAFields\Sections\Meta ) {
            $html = $this->renderMetaField();
        } else if ( AVA_Fields()->section( $this->container_id, $this->section_id ) instanceof \AVAFields\Sections\Options ) {
